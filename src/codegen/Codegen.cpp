...
/**
 * @brief 'type' always refers to llvm::Type, ast::TypeIdentifier is refered to as TypeIdentifier
 *
 * @param type
 * @return llvm::Type*
 */
llvm::Type*
Codegen::get_type(std::string const& type)
{
	auto Ty = get_builtin_type(type);
	if( Ty )
	{
		return Ty;
	}

	auto type_identifier = current_scope->get_identifier(type);
	...
	return type_identifier->data.type.TypeTy;
}
