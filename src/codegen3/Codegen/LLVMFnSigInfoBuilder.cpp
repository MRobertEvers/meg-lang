...
LLVMFnSigInfoBuilder::LLVMFnSigInfoBuilder(std::string name, ir::Type const* sema_fn_ty)
	: name(name)
...
void
LLVMFnSigInfoBuilder::add_arg_type(std::string name, LLVMArgABIInfo info)
{
	add_arg_type(info);
...
cg::get_llvm_arg_types(std::vector<LLVMArgABIInfo> const& abis)
{
