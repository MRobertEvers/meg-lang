...
In this branch I did some more exploration of how pretty printing works. I spent some time thinking about it and then I took a look at prettier (the js pretty printer). One problem I was having was how to handle comments, and generally things that don't appear in the AST. For the most part, I came up with the same stuff that prettier did. Their AST keeps track of comments as 'metadata', similar to how we keep track of token spans already.

There are several similarities between the road I was going down and the and what prettier does.

I noticed that 'trailing' and 'leading' comments were something that I likely needed to keep track of and prettier does this exactly. So I'm on the right track.

I created a 'formatting' Syntax Tree for pretty printing, which is what prettier does. Even certain behaviors of the tree, such as propagating line breaks to parent nodes. 
